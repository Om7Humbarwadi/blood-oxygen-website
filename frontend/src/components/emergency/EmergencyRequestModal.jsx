  ...
  return (
    <BaseModal title="Emergency Request Details" onClose={onClose} maxWidth="max-w-2xl">
      ...
      <div className="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm sm:grid-cols-2">
        ...
        <p><span className="font-semibold text-slate-700">Blood Group:</span> {request.bloodGroup}</p>
      ...
      <div className="mt-4 space-y-3">
        ...
        <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
          <input value={rejectNotes} onChange={(e) => setRejectNotes(e.target.value)} placeholder="Reject reason (optional)" className="rounded-lg border border-slate-300 px-3 py-2 text-sm" />
          <button type="button" onClick={onReject} disabled={loading} className="rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-70">Reject</button>
        </div>
      </div>
    </BaseModal>
  );
